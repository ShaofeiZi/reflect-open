...
export function CustomFilterMenu({
  ...
  let emptyMessage = t('allNotes.custom.no-matching-tags')
  if (typed === '') {
    emptyMessage = t('allNotes.custom.type-to-filter')
  } else if (!isTagName(typed)) {
    emptyMessage = t('allNotes.custom.invalid-tag')
  }
  ...
  return (
    ...
    >
      ...
      >
        {activeTag !== null ? `#${activeTag}` : t('allNotes.custom.label')}
      ...
      <PopoverContent align="end" sideOffset={6} className="w-56 p-0">
        <Command label={t('allNotes.custom.filter-by-another-tag')}>
          <CommandInput
            value={query}
            onValueChange={setQuery}
            placeholder={t('allNotes.custom.filter-placeholder')}
          />
          <CommandList>
            ...
            {offerTyped ? (
              ...
              <CommandGroup forceMount>
                <CommandItem forceMount value={`custom:${typed}`} onSelect={() => choose(typed)}>
                  <span className="min-w-0 flex-1 truncate">{t('allNotes.custom.filter-by-typed', { tag: typed })}</span>
                </CommandItem>
              </CommandGroup>
            ) : null}
          </CommandList>
