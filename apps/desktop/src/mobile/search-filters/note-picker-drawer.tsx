  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerContent>
        ...
        <SearchInput
          placeholder={t('mobile.filters.find-note')}
          aria-label={t('mobile.filters.find-note-aria')}
          value={query}
          onValueChange={setQuery}
        />
        <div className="min-h-0 flex-1 overflow-y-auto">
          {hits !== undefined && hits.length === 0 && (
            <p className="py-6 text-center text-sm text-text-muted">{t('mobile.filters.no-matches')}</p>
          )}
          {(hits ?? []).map((hit) => (
            <button
              key={hit.path}
