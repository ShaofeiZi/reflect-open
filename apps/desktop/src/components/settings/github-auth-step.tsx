...
export function GithubAuthStep({ onAuthed, repoName }: GithubAuthStepProps): ReactElement {
  ...
  async function verifyAndFinish(): Promise<void> {
    ...
    if (user === null) {
      throw new Error(t('settings.githubAuth.rejected'))
    }
    reportAuthed(user)
  }
  ...
  async function savePat(): Promise<void> {
    ...
    if (token.length === 0) {
      pat.setError(t('settings.githubAuth.pasteTokenFirst'))
      return
    }
    ...
    <div className="flex flex-col gap-3">
      {flowView.view === 'idle' ? (
        !usePat ? (
          ...
          <>
            ...
            <Button onClick={() => void savePat()} disabled={pat.pending} size="sm">
              {pat.pending ? t('settings.githubAuth.checking') : t('settings.githubAuth.saveToken')}
            </Button>
            {isDeviceFlowConfigured() ? (
              <button
                type="button"
              ...
              >
                {t('settings.githubAuth.signInInstead')}
              </button>
            ) : null}
          </>
        )
        ...
        <div className="flex flex-col gap-2">
          <p className="text-xs text-text-muted">
            {copyState === 'copied'
              ? t('settings.githubAuth.codeCopied')
              : t('settings.githubAuth.codePrompt')}
          </p>
          ...
          <Button size="sm" onClick={() => void copyCodeAndOpen(flowView)}>
            {copyState === 'failed' ? t('settings.githubAuth.openGithub') : t('settings.githubAuth.copyAndOpen')}
          </Button>
          {copyState === 'failed' ? (
            <p className="text-xs text-text-muted">
              {t('settings.githubAuth.copyFailed')}
            </p>
          ) : null}
          {openFailed ? (
            <p className="select-text text-xs text-text-muted">
              {t('settings.githubAuth.openFailed', { url: flowView.verificationUri })}
            </p>
          ) : null}
        </div>
      )}
