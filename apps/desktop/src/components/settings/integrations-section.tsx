  ...
  return (
    <SettingsSection id="integrations">
      {contactsAvailable ? (
        <div>
          <SettingsSwitchField
            legend={t('settings.integrationsSection.contactsLegend')}
            description={t('settings.integrationsSection.contactsDescription')}
          ...
          {showDenied ? (
            <div className="px-4 pb-3.5">
              <InlineAlert tone="warning">
                {t('settings.integrationsSection.contactsDeniedBefore')}{' '}
                <button
                  type="button"
                ...
                {t('settings.integrationsSection.contactsDeniedAfter')}
              </InlineAlert>
            </div>
          ) : null}
          {showPrompt ? (
            <div className="px-4 pb-3.5">
              <InlineAlert tone="warning">
                {t('settings.integrationsSection.contactsPromptBefore')}{' '}
                <button
                  type="button"
                  className="font-medium underline underline-offset-2"
                  onClick={() => void promptForAccess()}
                >
                  {t('settings.integrationsSection.allowContacts')}
                </button>
              </InlineAlert>
            </div>
          ) : null}
