...
export function SearchSection(): ReactElement {
  ...
  if (!settings.semanticSearchEnabled) {
    ...
    control = (
      ...
      >
        ...
        {t('settings.searchSection.enable')}
      </button>
    )
    ...
    control = (
      <div className="flex items-center justify-between gap-4">
        <span className="flex items-center gap-2 text-xs text-text-muted">
          ...
          {t('settings.searchSection.modelDownloaded', { model: status.model })}
        </span>
        <button
          type="button"
        ...
        >
          {t('settings.searchSection.disable')}
        </button>
      </div>
    )
    ...
    control = (
      <div>
        <InlineAlert tone="error">
          {t('settings.searchSection.loadFailed', { message: status.message })}
        </InlineAlert>
        <div className="mt-2 flex items-center gap-2">
          ...
          >
            {t('settings.searchSection.tryAgain')}
          </button>
          <button
            type="button"
            onClick={() => updateSettings({ semanticSearchEnabled: false })}
  ...
  return (
    <SettingsSection id="search">
      <SettingsField
        legend={t('settings.searchSection.semantic.legend')}
        description={t('settings.searchSection.semantic.description')}
      >
