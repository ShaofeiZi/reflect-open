  ...
  return (
    ...
    >
      <header className="flex h-11 shrink-0 items-center gap-1 border-b border-border pl-4 pr-1">
        <h1 className="min-w-0 flex-1 truncate text-base font-semibold">{t('sidebar.chat')}</h1>
        {hasProvider ? (
          <>
            {turns.length > 0 ? (
              <Button variant="ghost" size="icon" aria-label={t('chat.new-chat')} onClick={newChat}>
                <Plus aria-hidden />
              </Button>
            ) : null}
            <Button
              variant="ghost"
              size="icon"
              aria-label={t('mobile.chat-history.aria')}
      ...
      {hasProvider ? (
        ...
        <div className="flex flex-1 items-center justify-center px-6">
          <div className="flex max-w-sm flex-col items-center text-center">
            ...
            <h2 className="mt-4 text-lg font-semibold text-text">{t('chat.empty-heading')}</h2>
