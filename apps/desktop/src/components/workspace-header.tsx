  ...
  return (
    <header className="flex items-center justify-between gap-4 border-b border-black/10 px-6 py-3 dark:border-white/10">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {resolvedTheme === 'dark' ? t('common.header.light-mode') : t('common.header.dark-mode')}
        </button>
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              type="button"
              aria-label={t('common.aria.open-settings')}
          ...
          <TooltipContent>
            {t('common.header.settings-tooltip')} {SETTINGS_BINDING && <ShortcutKeys binding={SETTINGS_BINDING} />}
          </TooltipContent>
        </Tooltip>
      </div>
    </header>
