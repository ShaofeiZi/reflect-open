...
export function RecordingDrawer(): ReactElement {
  ...
  return (
    <Drawer open={memo.drawerOpen} onOpenChange={memo.onDrawerOpenChange}>
      <DrawerContent aria-label={t('mobile.recording.title')}>
        <DrawerTitle className="sr-only">{t('mobile.recording.title')}</DrawerTitle>
        {memo.error !== null ? (
  ...
  return (
    <div className="flex flex-col items-center gap-4 px-4 pb-2 text-center">
      <p className="text-sm text-text-muted">
        {t('mobile.recording.description')}
      </p>
      <Button
        onClick={() => {
      ...
      >
        {t('mobile.recording.open-settings')}
      </Button>
    </div>
  )
}
    ...
    <div className="flex flex-col items-center gap-4 pb-2">
      <div className="flex h-7 items-center justify-center">
        {memo.phase === 'recording' ? (
          ...
          <p className="text-sm text-text-muted">{t('mobile.recording.waiting-mic')}</p>
        )}
      </div>
      ...
      <div className="flex w-full flex-col items-center gap-3">
        <Button
          variant="destructive"
          size="icon"
          aria-label={t('audioMemo.stop-recording')}
        ...
        >
          ...
          {discardArmed ? t('mobile.recording.tap-again-discard') : t('audioMemo.discard')}
        </Button>
      </div>
    </div>
  )
