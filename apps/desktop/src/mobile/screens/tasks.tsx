  ...
  return (
    ...
    >
      <header className="flex shrink-0 items-center gap-1 border-b border-border px-4 pb-2 pt-1">
        <SearchInput
          ref={searchInputRef}
          placeholder={t('mobile.tasks-screen.search-placeholder')}
          aria-label={t('mobile.tasks-screen.search')}
          value={query}
        ...
        {recentlyCompleted.length > 0 ? (
          <Button
            ...
            aria-label={t('mobile.tasks-screen.archive-completed', { count: recentlyCompleted.length })}
        ...
        <Button
          ...
          size="icon"
          className="size-10 shrink-0"
          aria-label={t('mobile.tasks-screen.filters')}
          onClick={() => {
            hapticImpactLight()
            setFiltersOpen(true)
          }}
      ...
      {isError ? (
        <p role="alert" className="px-4 py-6 text-sm text-text-muted">
          {t('mobile.tasks-screen.load-failed')}
        </p>
        ...
        <div className="flex flex-1 items-center justify-center" aria-label={t('mobile.tasks-screen.loading')}>
          <Spinner className="size-5 text-text-muted" />
        </div>
      ) : groups.length === 0 ? (
        <div className="flex flex-1 flex-col items-center justify-center gap-3 text-text-muted">
          <CircleCheck className="size-6" />
          <p className="text-sm">{needle ? t('mobile.tasks-screen.no-matching') : t('mobile.tasks-screen.no-tasks')}</p>
          {needle === '' ? (
            <Button variant="outline" onClick={() => onAdd(todaysDailyTarget(today))}>
              {t('mobile.tasks-screen.add-task')}
            </Button>
          ) : null}
        </div>
      ) : (
      ...
      <Button
        size="icon"
        aria-label={t('mobile.tasks-screen.new-task')}
