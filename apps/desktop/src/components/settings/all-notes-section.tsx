...
export function AllNotesSection(): ReactElement {
  ...
  const addTag = (): void => {
    ...
    if (!isTagName(tag)) {
      setDraftError(t('settings.allNotesSection.invalidTag', { tag }))
      return
    }
    setDraft('')
  ...
  return (
    <SettingsSection id="all-notes">
      <SettingsField
        legend={t('settings.allNotesSection.filterTags.legend')}
        description={t('settings.allNotesSection.filterTags.description')}
      >
        <ul className="mt-3 flex flex-wrap items-center gap-1.5">
          {tags.map((tag) => (
            ...
            >
              ...
              <button
                type="button"
                aria-label={t('settings.allNotesSection.removeTag', { tag })}
          ...
          {tags.length === 0 ? (
            <li className="text-[13px] text-text-muted">
              {t('settings.allNotesSection.empty')}
            </li>
          ) : null}
        </ul>
        <form
        ...
        >
          <input
            ...
            aria-label={t('settings.allNotesSection.addAria')}
            aria-invalid={draftError !== null}
            placeholder={t('settings.allNotesSection.placeholder')}
          ...
          >
            {t('settings.allNotesSection.add')}
          </button>
        </form>
        {draftError !== null ? (
