...
export function TasksScreen(): ReactElement {
  ...
  return (
    <div
      ref={rootRef}
      tabIndex={-1}
      aria-label={t('tasks.screen.aria-label')}
      ...
      <header className="flex flex-none items-center gap-2 border-b border-border py-2.5 pl-2 pr-3 lg:pl-10">
        <div className="window-drag-control min-w-0 flex-1">
          ...
          <Input
            ...
            placeholder={t('tasks.screen.search.placeholder')}
            aria-label={t('tasks.screen.search.aria-label')}
        ...
        {selection.selectedCount > 0 ? (
          ...
          >
            <Button
              type="button"
              variant="ghost"
              aria-label={t('tasks.screen.schedule.aria-label', { count: selection.selectedCount })}
              className="window-drag-control text-xs text-text-muted"
            >
              <CalendarClock aria-hidden className="size-3.5" />
              {t('tasks.screen.schedule.button')}
        ...
        {selection.selectedCount > 0 ? (
          <Button
            type="button"
            variant="ghost"
            aria-label={t('tasks.screen.convert.aria-label', { count: selection.selectedCount })}
            onClick={onConvertToBullet}
            title={t('tasks.screen.convert.title')}
            className="window-drag-control text-xs text-text-muted"
          >
            <List aria-hidden className="size-3.5" />
            {t('tasks.screen.convert.button')}
            <TaskToolbarCountBadge count={selection.selectedCount} />
          </Button>
        ) : null}
        {recentlyCompleted.length > 0 ? (
          <Button
            type="button"
            variant="ghost"
            aria-label={t('tasks.screen.archive.aria-label', { count: recentlyCompleted.length })}
            onClick={actions.archive}
            className="window-drag-control text-xs text-text-muted"
          >
            <Archive aria-hidden className="size-3.5" />
            {t('tasks.screen.archive.button')}
      ...
      >
        {isError ? (
          <p role="alert" className="px-4 py-6 text-sm text-text-muted lg:px-12">
            {t('tasks.screen.error.load-failed')}
          </p>
        ) : ready && groups.length === 0 ? (
          <p className="px-4 py-6 text-sm text-text-muted lg:px-12">
            {needle ? t('tasks.screen.empty.no-matching') : t('tasks.screen.empty.no-tasks')}
          </p>
        ) : (
