...
export function ImportSection(): ReactElement {
  ...
  async function chooseAndImport(): Promise<void> {
    const result = await open({
      multiple: false,
      directory: false,
      title: t('settings.importSection.dialogTitle'),
      filters: [{ name: t('settings.importSection.zipArchives'), extensions: ['zip'] }],
    })
  ...
  return (
    <SettingsSection id="import">
      <SettingsField
        legend={t('settings.importSection.reflectV1.legend')}
        description={t('settings.importSection.reflectV1.description')}
      >
        <div className="mt-2">
          <Button
            type="button"
          ...
          >
            ...
            {running ? t('settings.importSection.importing') : t('settings.importSection.importZip')}
          </Button>
        </div>
      </SettingsField>
    </SettingsSection>
